...
export default function ReservationForm({ branches = [] }) {
  ...
  const [form, setForm] = useState({ name:'', email:'', phone:'', date:'', time:'', guests:'2', occasion:'', seating:'', requests:'', branch:'' });
  ...
  useEffect(() => {
    if (branches && branches.length > 0 && !form.branch) {
      setForm(f => ({ ...f, branch: branches[0].name }));
    }
  }, [branches]);
  ...
  if (sent) {
    return (
      <div className="glass-card rounded-3xl p-12 text-center">
        ...
        <p className="text-muted mb-2">Thank you, <strong>{form.name}</strong>! We&apos;ll confirm your table at our <strong>{form.branch}</strong> branch on <strong>{form.date}</strong> at <strong>{form.time}</strong> for <strong>{form.guests} guests</strong>.</p>
  ...
  return (
    <div className="glass-card rounded-3xl p-8">
      ...
      <form onSubmit={submit} className="space-y-6">
        <div className="grid md:grid-cols-2 gap-4">
          {branches.length > 0 && (
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold mb-2 text-heading">Select Branch *</label>
              <select className="form-input" required value={form.branch} onChange={e=>setForm({...form,branch:e.target.value})}>
                <option value="">Select a branch near you</option>
                {branches.map(b => <option key={b._id} value={b.name}>{b.name}</option>)}
              </select>
            </div>
          )}
          <div><label className="block text-sm font-semibold mb-2 text-heading">Full Name *</label><input type="text" className="form-input" required value={form.name} onChange={e=>setForm({...form,name:e.target.value})} /></div>
          <div><label className="block text-sm font-semibold mb-2 text-heading">Phone *</label><input type="tel" className="form-input" required value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} /></div>
          <div><label className="block text-sm font-semibold mb-2 text-heading">Email *</label><input type="email" className="form-input" required value={form.email} onChange={e=>setForm({...form,email:e.target.value})} /></div>
          <div><label className="block text-sm font-semibold mb-2 text-heading">Number of Guests *</label>
          ...
          <div><label className="block text-sm font-semibold mb-2 text-heading">Date *</label><input type="date" className="form-input" required min={today} value={form.date} onChange={e=>setForm({...form,date:e.target.value})} /></div>
          <div><label className="block text-sm font-semibold mb-2 text-heading">Time *</label>
        ...
        <div><label className="block text-sm font-semibold mb-2 text-heading">Occasion</label>
        ...
        <div><label className="block text-sm font-semibold mb-2 text-heading">Seating Preference</label>
        ...
        <div><label className="block text-sm font-semibold mb-2 text-heading">Special Requests</label><textarea className="form-input" rows="3" placeholder="Dietary requirements, allergies, etc." value={form.requests} onChange={e=>setForm({...form,requests:e.target.value})}></textarea></div>
